...
- [ ] `BUILD_SKILL` **corpus-snapshot — incumbency-confirmed un-saturated, ship after llm-expectations ships clean** — Re-checked 2026-05-17 incumbency: Datafold data-diff is SQL-warehouse only (no LLM/RAG/text port); no vector-store-diff tool exists on PyPI/GitHub; LangChain + LlamaIndex don't ship corpus-diff primitives; Pinecone/Weaviate/Qdrant offer backups not diffs; Evidently explicitly tells users to do this work manually. Langfuse / Phoenix / Helicone / TruLens are all trace-level, not corpus-level. **The wedge: "git status for your RAG corpus."** v0 scope: `corpus-snapshot snapshot ./docs` writes `.corpus-snapshot/state.json` (path + chunk hash + size + mtime); `corpus-snapshot diff <a> <b>` outputs markdown report. Local FS only, JSONL + markdown chunkers built in. Composes with llm-expectations: snapshot is the "what" that expectations runs against. v0.2 defers: pluggable iterator adapter, Pinecone/Weaviate fetch, embedding-aware diff, GitHub Action.
